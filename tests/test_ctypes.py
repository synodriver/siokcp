...
class TestCtypes(TestCase):
    ...
    def test_kcp_func(self):
        ...
        intptr = PyCapsule_GetPointer(
            ctypes.py_object(ikcp_create), b"siokcp._kcp.ikcp_create"
        )
        voidptr = ctypes.cast(intptr, ctypes.c_void_p)
        ikcp_create_t = ctypes.CFUNCTYPE(
            ctypes.POINTER(Kcp), ctypes.c_uint32, ctypes.c_void_p
        )
        ikcp_create = ctypes.cast(voidptr, ikcp_create_t)

        intptr = PyCapsule_GetPointer(
            ctypes.py_object(ikcp_release), b"siokcp._kcp.ikcp_release"
        )
        voidptr = ctypes.cast(intptr, ctypes.c_void_p)
